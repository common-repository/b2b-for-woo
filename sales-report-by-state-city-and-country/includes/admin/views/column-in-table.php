...
function devsoul_modify_user_columns( $column_headers ) {
	...
	if (!empty(get_option('devsoul_psbsp_show_total_sales_in_user_table'))) {
...
function devsoul_add_custom_product_column( $columns ) {
	if (!empty(get_option('devsoul_psbsp_stats_product_in_product_columns'))) {
...
function devsoul_custom_product_column_content( $column, $post_id ) {
	if ('total_profit' == $column) {
		...
		devsoul_product_detail($post_id, false);
...
function devsoul_product_detail( $post_id, $show_table = true ) {
	global $wpdb;
	...
	<div class="devsoul-order-detail">
		...
		<div id="ordersPopup" class="orders-popup-content" style="display:none;">
			...
			<?php
			include DEVS_PSBSCCP_PLUGIN_DIR . 'includes/admin/views/class-single-product-sales-report-table.php';
			// Initialize the table with the results data
			$table = new Single_Product_Sales_Report_Table($results);

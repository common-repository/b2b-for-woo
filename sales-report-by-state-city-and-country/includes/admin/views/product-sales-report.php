	...
	<div class="ct-psbsp-table-and-search-filed">
		...
		<div class="ct-psbsp-prouct-table" style="width: <?php echo esc_attr($width); ?>;">
			...
			$args = array(
				'status' => 'publish',
				'return' => 'ids',
				'limit' => !empty(get_option('devsoul_psbsp_product_per_page_for_product_sales')) ? get_option('devsoul_psbsp_product_per_page_for_product_sales') : 30,
				'order' => 'ASC',
			...
			include DEVS_PSBSCCP_PLUGIN_DIR . 'includes/admin/views/class-product-sales-report-table.php';

			$product_sales_table = new Product_Sales_Report_Table($products, $order_paramenters);
			$product_sales_table->prepare_items();

			$product_sales_table->display(); // Display the table
			$product_sales_table->display_totals(); // Display the totals below the table

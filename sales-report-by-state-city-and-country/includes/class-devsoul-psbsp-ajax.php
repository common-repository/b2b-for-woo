...
add_action('wp_ajax_update_value_exported', 'update_value_exported');
add_action('wp_ajax_devsoul_psbsp_product_search', 'devsoul_psbsp_product_search');
add_action('wp_ajax_devsoul_psbsp_category_search', 'devsoul_psbsp_category_search');
add_action('wp_ajax_devsoul_psbsp_customer_search', 'devsoul_psbsp_customer_search');
add_action('wp_ajax_devsoul_send_email_with_sales_detail', 'devsoul_send_email_with_sales_detail');
add_action('wp_ajax_change_order_status_of_selected_order', 'change_order_status_of_selected_order');
add_action('wp_ajax_ct_psbsp_show_graph', 'ct_psbsp_show_graph');

function update_value_exported() {
...
function devsoul_psbsp_product_search() {
	$nonce = isset($_POST['nonce']) ? sanitize_text_field(wp_unslash($_POST['nonce'])) : 0;
...
function devsoul_psbsp_category_search() {
	$nonce = isset($_POST['nonce']) ? sanitize_text_field(wp_unslash($_POST['nonce'])) : 0;
...
function devsoul_psbsp_customer_search() {
	$nonce = isset($_POST['nonce']) ? sanitize_text_field(wp_unslash($_POST['nonce'])) : 0;
...
function change_order_status_of_selected_order() {
	$nonce = isset($_POST['nonce']) ? sanitize_text_field(wp_unslash($_POST['nonce'])) : 0;
...
function devsoul_send_email_with_sales_detail() {
	...
	if (isset($_POST['ct_export_sales_csv'])) {
		...
		$enable_order = get_option('devsoul_psbsp_email_csv_sales_by_order_enable');
		$recipient_order = get_option('devsoul_psbsp_email_csv_sales_by_order_recipient');
		$subject_order = get_option('devsoul_psbsp_email_csv_sales_by_order_subject', 'Sale By Order');
		$additional_content_order = get_option('devsoul_psbsp_email_csv_sales_by_order_additional_content', 'Sale By Order');

		if (!empty($enable_order) && str_contains($file_name, 'order')) {
			$recipients = !empty($recipient_order) ? explode(',', $recipient_order) : array();
			$message_order = !empty($additional_content_order) ? $additional_content_order : 'Sale By Order';

			$attachment = DEVS_PSBSCCP_PLUGIN_DIR . 'assets/export-csv/export.csv';
			wp_mail($recipients, $subject_order, $message_order, $headers, $attachment);
		}

		// Handling email CSV Sale by Product settings
		$enable_product = get_option('devsoul_psbsp_email_csv_sales_by_product_enable');
		$recipient_product = get_option('devsoul_psbsp_email_csv_sales_by_product_recipient');
		$subject_product = get_option('devsoul_psbsp_email_csv_sales_by_product_subject', 'Sale By Product');
		$additional_content_product = get_option('devsoul_psbsp_email_csv_sales_by_product_additional_content', 'Sale By Product');

		if (!empty($enable_product) && str_contains($file_name, 'product')) {
			$recipients = !empty($recipient_product) ? explode(',', $recipient_product) : array();
			$message_product = !empty($additional_content_product) ? $additional_content_product : 'Sale By Product';

			$attachment = DEVS_PSBSCCP_PLUGIN_DIR . 'assets/export-csv/export.csv';
			wp_mail($recipients, $subject_product, $message_product, $headers, $attachment);
		}
		exit;

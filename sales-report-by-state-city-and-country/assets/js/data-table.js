jQuery(document).ready(function ($) {
    ...
    $(document).on('click', 'input[name=ct_export_sales_csv]', function (e) {
        ...
        if ($('.select_current_order').length) {
            ...
            let order_ids_array = [];
            var sum_of_item_ids_array = [];
            sum_of_item_ids_array['subtotal'] = 0;
            sum_of_item_ids_array['tax_total'] = 0;
            sum_of_item_ids_array['shipping_total'] = 0;
            sum_of_item_ids_array['refunded_amount'] = 0;
            sum_of_item_ids_array['coupon_amount'] = 0;
            sum_of_item_ids_array['total'] = 0;
            ...
            selecetd_checkbox.closest('tr').find('.ct-psbsp-order-complete-detail').each(function () {
                ...
                sum_of_item_ids_array['subtotal'] += parseFloat($(this).data('total') || 0);
                sum_of_item_ids_array['tax_total'] += parseFloat($(this).data('tax_total') || 0);
                sum_of_item_ids_array['shipping_total'] += parseFloat($(this).data('shipping_total') || 0);
                sum_of_item_ids_array['refunded_amount'] += parseFloat($(this).data('refunded_amount') || 0);
                sum_of_item_ids_array['coupon_amount'] += parseFloat($(this).data('coupon_amount') || 0);
                sum_of_item_ids_array['total'] += parseFloat($(this).data('total') || 0);

            });


            // Log key-value pairs from sum_of_item_ids_array
            Object.keys(sum_of_item_ids_array).reduce(function (accumulator, key) {
                var index = formatKey(key);
                var value = sum_of_item_ids_array[key];
                csvContent.push([index, value]);

                return accumulator; // Return the accumulator to maintain the pattern
            }, []);

            console.log($('.select_current_order:checked').val());
            console.log(sum_of_item_ids_array);
            console.log(csvContent);
        } else if ($('.selected_product').length) {
            var heading = [
                'Product id',
                'Product',
                'Price',
            ...
            let selecetd_checkbox = $('.selected_product').is(':checked') ? $('.selected_product:checked') : $('.selected_product');
            selecetd_checkbox.closest('tr').find('.ct-psbsp-order-complete-detail').each(function () {

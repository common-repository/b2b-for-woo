jQuery('document').ready(
	function ($) {
		...
		jQuery(document).on('click', '.ct-psbsp-filter-btn', function (e) {
			e.preventDefault();
			filter_for_sales();
		});
		...
		function get_fanilized_url() {
			...
			if (jQuery('.ct-psbsp-export-type').val()) {
				url = url + '&ct-psbsp-export-type=' + jQuery('.ct-psbsp-export-type').val();
			}
			if (jQuery('.ct-psbsp-select-user').val()) {
				url = url + '&customer=' + jQuery('.ct-psbsp-select-user').val();
			}
			if (jQuery('.ct-psbsp-select-order-status').val()) {
				url = url + '&order_status=' + jQuery('.ct-psbsp-select-order-status').val();
			}

			if (jQuery('.ct_psbsp_product_live_search').val()) {
				url = url + '&selected_product=' + jQuery('.ct_psbsp_product_live_search').val();
			}

			if (jQuery('.ct_psbsp_category_live_search').val()) {
				url = url + '&selected_cat=' + jQuery('.ct_psbsp_category_live_search').val();
			}

			if (jQuery('.ct-psbsp-select-countries').val()) {
				url = url + '&selected_countries=' + jQuery('.ct-psbsp-select-countries').val();
			}
			if (jQuery('.ct-psbsp-select-payment-method').val()) {
				url = url + '&select_payment_methods=' + jQuery('.ct-psbsp-select-payment-method').val();
			}
			if (jQuery('.ct-psbsp-select-shipping-method').val()) {
				url = url + '&select_shipping_methods=' + jQuery('.ct-psbsp-select-shipping-method').val();
			}
			if (jQuery('.ct-psbsp-select-state').val()) {
				url = url + '&selected_state=' + jQuery('.ct-psbsp-select-state').val();
			}
			if (jQuery('.ct-psbsp-enter-city').val()) {
				url = url + '&selected_cities=' + jQuery('.ct-psbsp-enter-city').val();
			}
			if (jQuery('.ct-psbsp-enter-post-code').val()) {
				url = url + '&postcode=' + jQuery('.ct-psbsp-enter-post-code').val();
			}

			if ('custom_Date' == select_date_type) {
				if (!jQuery('.ct-psbsp-start-date').val() && !jQuery('.ct-psbsp-end-date').val()) {
					jQuery('.ct-psbsp-start-date').focus();
					return;
				}

				if (jQuery('.ct-psbsp-start-date').val()) {
					url = url + '&start_date=' + jQuery('.ct-psbsp-start-date').val();
				}
				if (jQuery('.ct-psbsp-end-date').val()) {
					url = url + '&end_date=' + jQuery('.ct-psbsp-end-date').val();
				}
			}
...
jQuery(document).ready(function ($) {
	...
	$(document).on('click', '#showOrdersPopup', function (e) {
		e.preventDefault();
		$('.orders-popup-content').hide('slow');
	...
	$(document).on('click', '.orders-popup-close', function () {
		...
		$('.orders-popup-content').css('opacity', '0');
		setTimeout(function () {
			$('.orders-popup-content').hide('slow');

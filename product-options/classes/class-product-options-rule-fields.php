...
/**
 * Rule Level Option's Show
 */
class Product_Options_Rule_Fields {


	public static function ck_rule_prod_select( $rule_id ) {

		global $product;
		?>
		<div class="prod_optn_metabox">

			<div class="prod_optn_metabox_heading">
				<h3><b><?php echo esc_html('Products'); ?></b></h3>
			</div>

			<div class="prod_optn_metabox_description">
				<input type="checkbox" name="all_prod_select" class="all_prod_select" value="yes"
				<?php
				if ('yes' == get_post_meta($rule_id, 'all_prod_select', true)) {
					echo 'checked';
				}
				?>
				>
				<p><i><?php echo esc_html( 'Check if you want to show options on all products' ); ?></i></p>
			</div>
		</div>

		<div class="prod_optn_metabox pord_optn_spec_product_select_div">

			<div class="prod_optn_metabox_heading">
				<h3><b><?php echo esc_html('Specific Products'); ?></b></h3>
			</div>

			<div class="prod_optn_metabox_description">

				<?php
				$all_product_ids      = wc_get_products( array( 'status' => 'publish', 'limit' => -1, 'return' => 'ids' ) );
				$selected_product_ids = (array) get_post_meta($rule_id, 'prd_opt_prd_search', true);

				?>
				<select class="prd_opt_prd_search_class" name="prd_opt_prd_search[]" data-placeholder="Choose Products" multiple style="height: 40px; width: 80%;">

					<?php
					foreach ($all_product_ids as $pro_id) {

						$product_info = wc_get_product($pro_id);
						?>

						<option value="<?php echo esc_attr( $pro_id ); ?>"

							<?php echo in_array( $pro_id, $selected_product_ids) ? esc_attr('selected') : ''; ?>
						>

							<?php echo esc_attr($product_info->get_name()); ?>
				...
				<p><i><?php echo esc_html( 'Select Specific Products' ); ?></i></p>
			</div>
		</div>

		<div class="prod_optn_metabox pord_optn_spec_category_select_div">

			<div class="prod_optn_metabox_heading">
				<h3><b><?php echo esc_html('Specific Categories'); ?></b></h3>
			</div>
			...
			<div class="prod_optn_metabox_description">
				...
				$selected_category_ids = (array) get_post_meta($rule_id, 'prd_opt_cat_search', true);

				?>
				<select class="prd_opt_cat_search_class" name="prd_opt_cat_search[]" data-placeholder="Choose Categories" style="height: 40px; width: 80%;" multiple>
					<?php
					foreach ( $all_categories as $category ) {
						...
						if ( 'uncategorized' != $category->name) {
							...
							<option value="<?php echo esc_attr( $category_id ); ?>"
								...
								<?php echo in_array( $category_id, $selected_category_ids) ? esc_attr('selected') : ''; ?> >
				...
				<p><i><?php echo esc_html( 'Select Specific Categories' ); ?></i></p>
			</div>
		</div>

		<div class="prod_optn_metabox pord_optn_spec_tag_select_div">

			<div class="prod_optn_metabox_heading">
				<h3><b><?php echo esc_html('Specific Tags'); ?></b></h3>
			</div>

			<div class="prod_optn_metabox_description">
				<?php
				$all_tags          = get_categories( array( 'taxonomy' => 'product_tag' ) );
				$selected_tags_ids = (array) get_post_meta($rule_id, 'prd_opt_tag_search', true);
				?>
				<select name="prd_opt_tag_search[]" data-placeholder="Choose Tags" multiple class="prd_opt_tag" style="height: 40px; width: 80%;">
					<?php
					foreach ( $all_tags as $tag_info ) {
						...
						<option value="<?php echo esc_attr( $tag_id ); ?>"
							...
							<?php echo in_array( $tag_id, $selected_tags_ids ) ? esc_attr('selected') : ''; ?>

							>
							<?php echo esc_attr( $tag_info->name ); ?>
						</option>
						<?php
					}
					?>
				</select>
				<p><i><?php echo esc_html( 'Select Specific Tags' ); ?></i></p>
			</div>
		</div>
		<?php
	}

	public static function ck_rule_user_role( $rule_id ) {
		...
		<div class="prod_optn_metabox">
			...
			<div class="prod_optn_metabox_heading">
				<h3><b><?php echo esc_html('User Roles'); ?></b></h3>
			</div>

			<div class="prod_optn_metabox_description">

				<?php
				$user_role = (array) get_post_meta(get_the_ID(), 'user_roles', true);
				$roles     = $wp_roles->get_names();
				?>
				<select name="user_roles[]" class="user_roles" multiple data-placeholder='Choose User Role' style="height: 40px; width: 80%;">
					<?php
					$user_roles          = $roles;
					$user_roles['guest'] = 'Guest';

					foreach ($user_roles as $key => $u_role) {
						?>
						<option value="<?php echo esc_attr($key); ?>"
							<?php echo in_array( (string) $key, $user_role, true) ? esc_attr('selected') : ''; ?>>

							<?php echo esc_attr($u_role); ?>
						</option>
						<?php
					}
					?>
				</select>
				<p><i><?php echo esc_html('select desired user role'); ?></i></p>
				<p><i><?php echo esc_html('leave empty for all'); ?></i></p>
	...
	public static function ck_rule_fields( $rule_id ) {

		?>
		<div class="expand_close_btn_div">
		...
		<div class="fields_main_div">
			...
			foreach ( $field_ids as $field_id ) {
				...
				$required_checkbox    = get_post_meta($field_id, 'ck_req_field', true);
				$field_price_checkbox = get_post_meta($field_id, 'ck_field_price_checkbox', true);
				...
				<div class="field_div">
					...
					<div class="prod_optn_remove_field_btn_main_div display_flex <?php echo esc_attr($field_id); ?>_prod_optn_remove_field_btn_main_div">
						...
						<div class="prod_optn_remove_field_btn_div">
							...
							<input type="hidden" name="prod_optn_rule_id" class="prod_optn_rule_id" value="<?php echo esc_attr($rule_id); ?>">

							<input type="number" name="ck_field_priority[<?php echo esc_attr($field_id); ?>]" style="height: 40px; width: 30%; vertical-align: bottom;" min="0" placeholder="Field Priority" value="<?php echo esc_attr($field_priority); ?>">

							<button class="field_dlte_btn" title="Remove Field" data-field_id="<?php echo esc_attr($field_id); ?>" data-type="rule">
					...
					<div class="display_flex prod_opt_depend_main_div <?php echo esc_attr($field_id); ?>_prod_opt_depend_main_div ">
						...
						<div class="option_selector_div <?php echo esc_attr($field_id); ?>_option_selector_div">
							...
							<select name="ck_options_selector[<?php echo esc_attr($field_id); ?>]" class="option_selector <?php echo esc_attr($field_id); ?>_option_selector" multiple>
					...
					<div class="display_flex prod_opt_field_price_main_div <?php echo esc_attr($field_id); ?>_prod_opt_field_price_main_div">
						...
						<div style="width: 100%; float: left; margin: 5px">
							...
							<div class="prod_opt_field_price_type_price_div <?php echo esc_attr($field_id); ?>_prod_opt_field_price_type_price_div">
								...
								<input type="number" name="ck_field_price[<?php echo esc_attr($field_id); ?>]" value="<?php echo esc_attr($field_price); ?>" class="ck_field_price <?php echo esc_attr($field_id); ?>_ck_field_price" placeholder="Field Price" min="0">
					...
					<div class="display_flex prod_opt_limit_range_main_div <?php echo esc_attr($field_id); ?>_prod_opt_limit_range_main_div">
						...
						<div style="width: 100%; float: left; margin: 5px">
							...
							<div class="prod_opt_field_limit_to_div <?php echo esc_attr($field_id); ?>_prod_opt_field_limit_to_div">
								...
								<input type="number" name="ck_field_min_limit[<?php echo esc_attr($field_id); ?>]" value="<?php echo esc_attr($field_min_limit); ?>" class="ck_field_min_limit <?php echo esc_attr($field_id); ?>_ck_field_min_limit" placeholder="Minimum Limit" min="1">

								<span><?php echo esc_html('to'); ?></span>

								<input type="number" name="ck_field_max_limit[<?php echo esc_attr($field_id); ?>]" value="<?php echo esc_attr($field_max_limit); ?>" class="ck_field_max_limit <?php echo esc_attr($field_id); ?>_ck_field_max_limit" placeholder="Maximum Limit" min="1">
								...
								<tbody>
									...
									foreach ( $option_idss as $option_id ) {
										...
										<tr class="prod_optn_option_tr" data-field_id="<?php echo esc_attr($field_id); ?>" data-option_id="<?php echo esc_attr($option_id); ?>">
											...
											<td>
												...
												<div class="prod_opt_color_img_div <?php echo esc_attr($field_id); ?>_prod_opt_color_img_div">
													...
													<div class="prod_opt_color_div <?php echo esc_attr($field_id); ?>_prod_opt_color_div">
														...
														<input type="color" name="ck_option_color[<?php echo esc_attr($field_id); ?>][<?php echo esc_attr($option_id); ?>]" value="<?php echo esc_attr($option_color); ?>">
											...
											<td>
												<select name="ck_option_price_type[<?php echo esc_attr($field_id); ?>][<?php echo esc_attr($option_id); ?>]" class="ck_option_price_type">
												...
												<input type="number" name="ck_option_price[<?php echo esc_attr($field_id); ?>][<?php echo esc_attr($option_id); ?>]" class="ck_option_price" value="<?php echo esc_attr($option_price); ?>" placeholder="Option Price" min="0">
											</td>

											<td>
												<input type="number" name="ck_option_priority[<?php echo esc_attr($field_id); ?>][<?php echo esc_attr($option_id); ?>]" class="ck_option_priority" value="<?php echo esc_attr($option_priority); ?>" placeholder="Option Priority" min="0">
											</td>
					...
					<div class="prod_opt_add_optn_btn_div <?php echo esc_attr($field_id); ?>_prod_opt_add_optn_btn_div">
						...
						<button class="option_add_btn" title="Add Option" data-parent_id="<?php echo esc_attr($field_id); ?>" data-type="rule">
		...
		<div class="prod_opt_add_field_btn_main_div">
			...
			<div class="display_flex">
				...
				<div class="prod_opt_add_field_btn">
					...
					<button class="field_add_btn" title="Add Field" data-rule_id="<?php echo esc_attr($rule_id); ?>" data-type="rule">
				...
				<div class="prod_opt_csv_main_div">
						...
						<input type="hidden" name="current_rule_id" value="<?php echo intval($rule_id); ?>">
				</div>
...
new Product_Options_Rule_Fields();
